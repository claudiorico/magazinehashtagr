...
const CartItem = ({ id, qtd }) => {
    const { name, image, price } = catalogIndexedById[id];
    const { addToCart, subToCart, removeFromCart } = useCartContext();

    return <article className='flex bg-stone-100 p-1 border rounded-mg relative'>
        <img src={image} alt={`Produto-${id}`} className='h-24' />
        ...
        <div className='flex flex-col justify-around mx-2'>
            ...
            <div className='flex text-slate-950 absolute right-2 bottom-0 mb-2'>
                <button onClick={() => subToCart(id)} className='border border-slate-400 hover:border-slate-900 mb-2'>
                    <FontAwesomeIcon icon={faMinus} className='p-1' />
                </button>
                <p className='p-2 w-8 mx-auto'>{qtd}</p>
